...
class Agent:
    ...
    def train(self):
        ...
        def model_function(fixed_step_data_x, num):
            # if num == "single":
            input_x = fixed_step_data_x.reshape(-1, *fixed_step_data_x.shape)
            # else:
            #     input_x = fixed_step_data_x
            predicted_y = self.dqn_model(input_x)
            return predicted_y

        def gradient(X, fixed_step_data_x, fixed_step_data_y, num="single"):
            with tf.GradientTape(persistent=True) as tape:
                Y = model_function(fixed_step_data_x, num)
                loss = tf.reduce_mean(tf.square(fixed_step_data_y - Y))
            gradient = tape.gradient(loss, X)
            return gradient

        def question4(X_k, X_kplus1, grad_f_k, grad_f_kplus1, H_k):
            s_k = (tf.identity(X_kplus1) - tf.identity(X_k)).numpy()
            v_k = (tf.identity(grad_f_kplus1) - tf.identity(grad_f_k)).numpy()
        ...
        def question1(X_0, model_function, data_x, data_y, lr=0.001, sgd_iters=10):
            ...
            for j in range(sgd_iters):
                ...
                g = gradient(X, data_x[k], data_y[k])
                grad_f_kplus1 = gradient(X, data_x[k + 1], data_y[k + 1])
                for i, x in enumerate(X):
                    H_kplus1 = question4(
                        X_0[i], X[i], g[i], grad_f_kplus1[i], np.eye(len(g[i]))
                    )
                    X[i].assign_sub(H_kplus1 * lr * g[i])
                # X.assign_sub(lr * g)
            return X

        # def question2(X_1, m, model_function, data_x, data_y, lr=0.001, svrg_iters=100):
        #     n = len(data_x)
        #     X_k = X_1
        #     for k in range(svrg_iters):
        #         grad_f_n = gradient(X_k, data_x, data_y, num="many")
        #         # print(grad_f_n.shape)
        #         X_k_j = tf.Variable(tf.identity(X_k))
        #         for j in range(1, m + 1):
        #             i = np.random.randint(0, n)
        #             g = gradient(X_k_j, data_x[i], data_y[i]) - (
        #                 gradient(X_k, data_x[i], data_y[i]) - grad_f_n
        #             )
        #             # print(g.shape)
        #             # X_k_j.assign_sub(lr * g)
        #             for i, x in enumerate(X_k_j):
        #                 X_k_j[i].assign_sub(lr * g[i])
        #             # if all(X_k == X_k_j):
        #             #     print("booo")
        #         X_k = tf.Variable(tf.identity(X_k_j))
        #     return X_k
        ...
        for i, x in enumerate(X):
            self.dqn_model.trainable_variables[i].assign(X[i])
